<template>
    <div class="ui massive search search-container">
        ...
        <div v-show="searching" class="search-results">
            <div v-if="results.length == 0" class="ui divided selection list" style="text-transform: capitalize">
            ...
            <div v-else>
                <div class="ui divided selection list capitalize">
                    <a v-for="result in results.slice(0, 5)" :key="result.slug" class="item" @click="redirectTo(result)">
                        ...
                        <div class="description">
                            <span v-if="result.resource_type == 'cookbook'" class="ui left floated">
                                {{ truncate(result.description, 115, '...') }}
                            </span>
                            <span v-if="result.resource_type == 'recipe'" class="ui left floated">
                                {{ truncate(result.summary, 115, '...') }}
                            </span>
                            <span v-if="result.resource_type == 'cookbook'" class="ui right floated meta">
                                <img class="ui mini image" :src="result.bookCoverImg" />
                            </span>
                            <span v-if="result.resource_type == 'recipe'" class="ui right floated meta">
                                <img class="ui mini image" :src="result.imgUrl" />
                            </span>
                            <span v-if="result.resource_type == 'recipe_variation'" class="ui right floated meta">
                                <img class="ui mini image" :src="result.imgUrl" />
                            </span>
                        </div>
                    </a>
